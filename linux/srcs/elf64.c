...
static char            print_type(Elf64_Sym sym)
{
	...
	Elf64_Word	sh_type = REV32(shdr_begin[st_shndx].sh_type);
	Elf64_Word	sh_flags = REV32(shdr_begin[st_shndx].sh_flags);
...
int	handle_elf64(char* file, const uint64_t filesize) {
	...
	shdr_begin = (Elf64_Shdr *)(file + REV64(hdr->e_shoff));
	...
	sectionNames_stringTable = (char *)(file + REV64(shdr_begin[REV64(hdr->e_shstrndx)].sh_offset));
	dprintf(2, "sectionNames_stringTable at %p\n", (void *)sectionNames_stringTable);

	for (Elf64_Half i = 0; i < REV16(hdr->e_shnum); i++) {
	...
	Elf64_Sym	*symbols = (Elf64_Sym *)(file + REV64(symboltable_sectionheader->sh_offset));
	str = (char *)(file + REV64(stringtable_sectionheader->sh_offset));
